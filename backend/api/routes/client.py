...
def start_client_device_session():
    try:
        ...
        with get_db_connection() as conn:
            ...
            # Allow device-session OTP challenges to be created before identity is unlocked.
            # We only block hard-locked invitations here.
            access_row = _identity_access_row(cursor, invitation_token, proposal_id)
            if access_row.get('locked_at') is not None:
                return {
                    'detail': 'Access locked due to too many failed identity attempts',
...
def get_client_proposals():
    ...
    try:
        ...
        try:
            ...
            if len(di_preview) > 12:
                di_preview = di_preview[:12] + '...'
            if len(st_preview) > 12:
...
def client_sign_proposal_in_app(proposal_id):
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            merged_pdf_bytes = _merge_proposal_pdf_and_signature_page(pdf_bytes, raw_png)
